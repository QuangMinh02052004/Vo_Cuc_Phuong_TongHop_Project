import { queryTongHop, queryOneTongHop, queryDatVe } from '../../../../../lib/database';
  ...
  try {
    ...
    if (!datveRouteId) {
      try {
        ...
        console.error('[tong-hop/routes/:id] auto-link failed:', err.message);
      }
    }
    ...
    if (datveRouteId) {
      try {
        ...
        console.error('[tong-hop/routes/:id] DatVe sync failed:', err.message);
      }
    }

    return NextResponse.json({ success: true, route: { ...updated, datveRouteId } });
  } catch (error) {
