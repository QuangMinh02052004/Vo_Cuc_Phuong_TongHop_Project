...
// === Reconciliation polling: 30s gọi backend để sync đơn dọc đường bị sót ===
// Nhanh hơn cron Vercel 15p, đảm bảo đơn dọc đường mới sót sẽ được sync trong <30s.
(function setupReconciliationPolling() {
  let lastResult = null;
  async function tick() {
    try {
      // Quét 6h gần nhất, tối đa 30 đơn/lần để khỏi tải nặng
      const res = await fetch('/api/nhap-hang/reconcile-doc-duong?since=360&max=30', { cache: 'no-store' });
      const json = await res.json();
      if (json?.success && json.created > 0) {
        console.log(`[Reconcile] Tạo bù ${json.created} booking dọc đường`);
      }
      lastResult = json;
    } catch (e) {
      console.warn('[Reconcile] Lỗi:', e.message);
    }
  }
  // Chạy lần đầu sau 5s (đợi page load xong), sau đó mỗi 30s
  setTimeout(tick, 5000);
  setInterval(tick, 30000);
  window.__lastReconcile = () => lastResult;
})();
